...
def push_as_commit(path, name, branch, username):
    ...
    # Push commit
    try:
        repo.git.push('ssh://' + username + '@review.slimroms.org:29418/' + name, 'HEAD:refs/for/' + branch + '%topic=translation')
...
parser.add_argument('-b', '--branch', help='SlimRom branch', required=True)
...
if not os.path.isfile('platform_manifest/default.xml'):
    ...
    print('Found: platform_manifest/default.xml')
