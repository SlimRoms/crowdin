...
def push_as_commit(path, name, branch, username):
    ...
    # Push commit
    try:
        repo.git.push('ssh://%s@review.slimroms.org:29418/%s' % (username, name),
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Synchronising SlimRoms's translations with Crowdin")
    sync = parser.add_mutually_exclusive_group()
    parser.add_argument('-u', '--username', help='Gerrit username',
                        required=True)
    parser.add_argument('-b', '--branch', help='SlimRoms branch',
                        required=True)
    sync.add_argument('--no-upload', action='store_true',
                      help='Only download SlimRoms translations from Crowdin')
    sync.add_argument('--no-download', action='store_true',
                      help='Only upload SlimRoms source translations to Crowdin')
...
def load_xml(x='platform_manifest/default.xml'):
